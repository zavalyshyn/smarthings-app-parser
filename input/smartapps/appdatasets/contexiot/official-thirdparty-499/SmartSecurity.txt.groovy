...
{
	intrusionMotions.each {
		subscribe(it.motion)
	}
	residentMotions.each {
		subscribe(it.motion)
	}
	intrusionContacts.each {
...
def checkForReArm()
{
	def threshold = (residentsQuietThreshold ? residentsQuietThreshold : 3) * 60 * 1000
	// check last intruder motion
	log.debug "checkForReArm, time since last intruder motion: ${now() - state.lastIntruderMotion}"
	if (now() - state.lastIntruderMotion > threshold) {
